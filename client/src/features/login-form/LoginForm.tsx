...
export function LoginForm() {
  ...
  return (
    <form className={styles.form} onSubmit={submit}>
      ...
      <AuthField id="email" type="email" autoComplete="email" required label={t("auth.email")} value={email} onChange={(event) => setEmail(event.target.value)} />
      <AuthField id="password" type="password" autoComplete="current-password" required label={t("auth.password")} value={password} onChange={(event) => setPassword(event.target.value)} />
