...
export function ProjectList({ workspace, onWorkspaceChange, heading }: ProjectListProps) {
  ...
  useEffect(() => { void window.ohmycode.projects.list().then(setProjects); }, []);

  function toggle(project: LocalProject) {
    onWorkspaceChange(project.path);
    setExpanded((current) => {
  ...
  async function createProject() {
    ...
    onWorkspaceChange(result.project.path);
  ...
  async function removeProject(project: LocalProject) {
    ...
    if (workspace === project.path) onWorkspaceChange(null);
  ...
  return <section className={styles.root}>
    ...
    <div className={styles.list}>
      {projects.map((project) => <article key={project.id} className={styles.project}>
        <div className={`${styles.projectRow} ${workspace === project.path ? styles.active : ""}`}>
        ...
        {expanded.has(project.id) && <div className={styles.conversations}>
          {project.conversations.map((conversation) => <div className={styles.conversation} key={conversation.id}>
            <Tooltip className={styles.conversationName} content={conversation.title}><button onClick={() => onWorkspaceChange(project.path)}>{conversation.title}</button></Tooltip>
