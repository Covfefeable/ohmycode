...
export async function executeMcpCapability(tool: string, args: Record<string, unknown>): Promise<unknown> {
  const serverToken = mcpToolServerToken(tool);
  if (!serverToken) throw new Error("invalid_mcp_tool");
  const server = (await listMcpServers()).find((item) => matchesMcpServer(item.id, serverToken) && item.enabled);
  const original = server?.tools.find((item) => createMcpToolName(server.id, item.name) === tool)?.name;
