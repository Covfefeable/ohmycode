...
import { UpdateSettings } from "../../features/update-settings";
...
export function SettingsPage() {
  ...
  const tab: SettingsTab = requestedTab === "models" || requestedTab === "background" || requestedTab === "mcp" || requestedTab === "skills" || requestedTab === "updates" ? requestedTab : "profile";
  ...
  if (tab !== "updates" && loadFailed) return <LoadError message={t("settings.loadFailed")} onRetry={() => { setLoadFailed(false); setReloadToken((value) => value + 1); }} />;
  if (tab !== "updates" && !settings) return <FullScreenLoading />;
  const content = tab === "updates" ? <UpdateSettings />
    : tab === "profile" ? <ProfileSettings initial={settings!.profile} tokenUsage={settings!.tokenUsage} />
